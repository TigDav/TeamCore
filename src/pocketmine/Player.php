...
class Player extends Human implements CommandSender, InventoryHolder, ChunkLoader, IPlayer {
	...
	protected function processLogin(){
        foreach($this->server->getOnlinePlayers() as $p){
            if (strtolower($this->username) == strtolower($p->getName()))
            {
                $this->close($this->getLeaveMessage(), "§r§8[§l§cЗащита§r§8] §l§fИгрок с таким ником §cуже играет §l§fна §eэтом§f сервере!§r");
                return true;
            }
        }
				...
				switch($packet->status){
					...
					case ResourcePackClientResponsePacket::STATUS_SEND_PACKS:
						...
						foreach($packet->packIds as $uuid){
							...
							$pk->maxChunkSize = 131072; //1MB   1048576
			...
			case ProtocolInfo::COMMAND_STEP_PACKET:
				...
				if($packet->inputJson !== null){
                        if(count($packet->inputJson) > 15){
                            $this->server->getNetwork()->blockAddress($this->getAddress(), 12);
                            $this->close("", "", false);
                            return;
                        }
			...
			case ProtocolInfo::TEXT_PACKET:
				...
				if($packet->type === TextPacket::TYPE_CHAT){
                    if(strlen($packet->message) > 350){
                        $this->server->getNetwork()->blockAddress($this->getAddress(), 12); //TODO: давать шанс игрокам которые набрали 350 символов..
                        break;
                    }
	...
	public final function close($message = "", $reason = "generic reason", $notify = true){
		if($this->connected and !$this->closed){
            ...
            foreach ($this->server->getOnlinePlayers() as $player){
                if (strtolower($player->getName()) == strtolower($this->username) and $player != $this){
                    $alreadyPlaying = true;
                    break;
                }
            }
//            if (file_exists('/root/players/' . strtolower($this->username) . '.dat') and file_get_contents('/root/players/' . strtolower($this->username) . '.dat') == $this->getServer()->getPort() and !$alreadyPlaying)
//            {
//                @unlink('/root/players/' . strtolower($this->username) . '.dat');
//            }
			$this->connected = false;
			if(strlen($this->getName()) > 0 and !$alreadyPlaying){
