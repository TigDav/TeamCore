...
class Server{
	...
	public function getiTXApiVersion(){
		return \pocketmine\TEAM_API_VERSION;
	}
...
─░█── ▀▀▀ ▀──▀ ▀───▀ ░█▄▄█ ▀▀▀▀ ▀─▀▀ ▀▀▀
	...
	public function __construct(\ClassLoader $autoloader, \ThreadedLogger $logger, $filePath, $dataPath, $pluginPath, $defaultLang = "unknown"){
		...
		try{
            ...
            if(file_exists($this->filePath . "src/pocketmine/resources/team_$lang.yml")){
                $content = file_get_contents($file = $this->filePath . "src/pocketmine/resources/team_$lang.yml");
            }else{
                $content = file_get_contents($file = $this->filePath . "src/pocketmine/resources/team_eng.yml");
            }
			...
			if($cfgVer > $advVer){
				$this->logger->notice("Ваш genisys.yml нуждается в обновлении");
	...
	public function start(){
		...
		if (!file_exists($this->getPluginPath() . DIRECTORY_SEPARATOR . "TeamCore")) {
			@mkdir($this->getPluginPath() . DIRECTORY_SEPARATOR . "TeamCore");
		}
